    ...
    async function handleDeductInventory() {
        ...
        try {
            ...
            const updated = freshInventory.map(item => {
                const d = active.find(d => d.item.item_name.toLowerCase() === item.item_name?.toLowerCase());
                if (!d) return item;
                return { ...item, count: Math.max(0, parseFloat(item.count ?? item.quantity ?? 0) - d.qty) };
            });
                                            ...
                                            deductions.length > 0 ? (
                                                <>
                                                    ...
                                                    <div className="cf-deduct-list">
                                                        {deductions.map((d, i) => {
                                                            const current = parseFloat(d.item.count ?? d.item.quantity ?? 0);
